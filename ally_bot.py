...
async def generate_openai(user_input: str, max_tokens=150, temperature=0.7):
    ...
    response = await asyncio.to_thread(
        ...
        model="gpt-4o-mini",  # cheaper + fast model for Discord bots
        messages=[
...
async def on_message(message):
    ...
    if is_dm or is_mention:
        ...
        if not content:
            return
        try:
            reply = await generate_openai(content)
        except Exception as e:
            await message.reply(f"⚠️ Error contacting OpenAI: {e}")
            return
